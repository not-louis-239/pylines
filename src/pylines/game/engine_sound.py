...
class SoundManager:
    ...
    def __init__(self, sounds: Sounds):
        ...
        self.engine_ch1 = pg.mixer.Channel(SFXChannelID.ENGINE1)
        self.engine_ch2 = pg.mixer.Channel(SFXChannelID.ENGINE2)

        self.engine_ch1.set_volume(1.0)
        self.engine_ch2.set_volume(0.0)
    ...
    def update(self, throttle_frac: float):
        ...
        if now < self.transition_end_time:
            ...
            self.engine_ch1.set_volume(progress)
            self.engine_ch2.set_volume(1.0 - progress)
        elif self.current_sound_ch2:
            # Transition is over. Promote ch2 to ch1.
            self.engine_ch1.stop()
            self.engine_ch1, self.engine_ch2 = self.engine_ch2, self.engine_ch1
