...
class BriefingScreen(State):
    ...
    def draw(self, wn: Surface):
        ...
        draw_text(self.display_surface, (C.WN_W//2, int(C.WN_H*0.075)), 'centre', 'centre', "Flight Briefing", (0, 192, 255), 40, self.fonts.monospaced)

        instructions: list[str] = self.game.assets.texts.briefing_text

        for i, line in enumerate(instructions):
            draw_text(
                self.display_surface, (C.WN_W // 2, int(C.WN_H * (0.17 + 0.04*i))), 'centre', 'centre',
