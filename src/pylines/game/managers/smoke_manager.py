...
class SmokeBlob:
    def __init__(self, x: RealNumber, y: RealNumber) -> None:
        ...
        vx = random.uniform(-1, 1) * SMOKE_DRIFT_FACTOR
        vy = -random.uniform(0.7, 1.3) * BASE_SMOKE_RISE_SPEED
        self.screen_vel: pg.Vector2 = pg.Vector2(vx, vy)

    def update(self, dt: int) -> None:
        dt_seconds = dt / 1000
        self.screen_pos += self.screen_vel * dt_seconds
...
class SmokeManager:
    ...
    def __init__(self, images: Images) -> None:
        ...
        for size in range(self.smoke_blob_cache_min, self.smoke_blob_cache_max + 1, 10):
    ...
    def update(self, dt: int) -> None:
        ...
        if len(self.smoke_blobs) < MAX_SMOKE_BLOBS:
    ...
    def draw_smoke_blobs(self, surface: Surface) -> None:
        blit_items = []
        for blob in self.smoke_blobs:
            surf_size = int(blob.diameter - (blob.diameter % 10))
            cached_surf: Surface | None = self.smoke_blob_cache.get(surf_size)
            blit_items.append((
                cached_surf,
                (int(blob.screen_pos.x - surf_size/2), int(blob.screen_pos.y - surf_size/2)),
                None,
                pg.BLEND_PREMULTIPLIED,
            ))
