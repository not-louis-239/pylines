...
void main() {
    ...
    vec4 treeline     = texture2D(treeline_rock_texture, v_tex_coord);
    vec4 alpine       = texture2D(alpine_rock_texture, v_tex_coord);
    ...
    float b_treeline   = smoothstep(treeline_rock_level - blend_range, treeline_rock_level + blend_range, v_height);
    float b_alpine     = smoothstep(alpine_rock_level   - blend_range, alpine_rock_level   + blend_range, v_height);
    ...
    color = mix(color, treeline,   b_treeline);
    color = mix(color, alpine,     b_alpine);
